...
import { getKakaoMapStatus, getKakaoMapErrorMessage } from "../../utils/kakaoMapUtils";
...
export default function CommonMap({ lat, lng, markers = [], height = "300px", level = 3 }) {
  ...
  // 카카오맵 로딩 상태 확인
  const mapStatus = getKakaoMapStatus();
        ...
        <div style={{ textAlign: "center" }}>
          ...
          {import.meta.env.DEV && (
            <div style={{ fontSize: "12px", color: "#666", marginTop: "8px" }}>
              <p>API 키: {mapStatus.apiKeySet ? "설정됨" : "설정되지 않음"}</p>
              <p>맵 로드: {mapStatus.mapLoaded ? "완료" : "대기 중"}</p>
              <p>서비스: {mapStatus.servicesAvailable ? "사용 가능" : "대기 중"}</p>
            </div>
          )}
        </div>
      </div>
  ...
  try {
    ...
    return (
      <div style={{ width: "100%", height, display: "flex", alignItems: "center", justifyContent: "center", backgroundColor: "#ffffff" }}>
        <div style={{ textAlign: "center" }}>
          <p>{getKakaoMapErrorMessage(error)}</p>
          {import.meta.env.DEV && (
