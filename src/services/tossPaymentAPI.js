  ...
  try {
    ...
    if (isRetryableError && retryCount < RETRY_CONFIG.maxRetries) {
      const delay =
        ...
        Math.pow(RETRY_CONFIG.retryBackoff, retryCount);
      logger.warn(
        `📡 토스페이먼츠 API 재시도 ${retryCount + 1}/${
...
class TossPaymentAPI {
  constructor() {
    ...
    if (!envClientKey) {
      logger.warn(
        "VITE_TOSS_CLIENT_KEY가 설정되지 않았습니다. 테스트 키를 사용합니다."
      );
  ...
  registerPaymentAttempt(orderId) {
    if (this.isPaymentInProgress(orderId)) {
      throw new Error(
        "이미 진행 중인 결제가 있습니다. 잠시 후 다시 시도해주세요."
      );
    }
  ...
  async createPayment(paymentInfo) {
    try {
      ...
      logger.log(
        "🔗 전체 URL:",
        `${API_CONFIG.BASE_URL}${API_ENDPOINTS.PAYMENT_CREATE}`
      );
  ...
  async createTestPayment(paymentInfo) {
    try {
      ...
      logger.log(
        "🌐 테스트 API 엔드포인트:",
        API_ENDPOINTS.PAYMENT_TEST_CREATE
      );
  ...
  async requestTossPayment(paymentData) {
    try {
      ...
      logger.log(
        "🔧 requestPayment 메서드:",
        typeof tossPayments.requestPayment
      );

      // 결제 요청
      const tossPayment = await tossPayments.requestPayment(
        "카드",
        requestData
      );
  ...
  async confirmPaymentLegacy(paymentData) {
    ...
    try {
      logger.log("📡 토스페이먼츠 결제 승인 요청 (백엔드 API):", {
        orderId,
        amount,
        paymentKey,
      });
